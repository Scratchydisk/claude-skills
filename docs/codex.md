...
Symlinking from this checkout changes the name Codex advertises — see [skill names under Codex](#skill-names-under-codex) below, which applies to every skill here, not only this one.
...
Codex resolves a symlinked skill directory and, when the target sits inside a plugin repository, advertises the skill under that plugin's name. This checkout carries `.claude-plugin/plugin.json` with `name: scratchydisk-skills`, so **every** skill symlinked from here — `spec-to-ship`, `contract-audit`, `devils-advocate-loop`, `karpathy-guidelines`, `anti-ai-tells`, `plantuml-diagrams`, `brainstorming` — is advertised as `scratchydisk-skills:<id>` rather than bare `<id>`. Copying a directory instead of symlinking it, or symlinking a target that sits outside a plugin repository, gives the bare `<id>`.

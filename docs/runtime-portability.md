...
A few limits are worth stating rather than glossing. First, a scope limit that applies to all three PASS rows above, not just Codex's: every row certifies that the host's own discovery machinery finds the skill and hands its model the exact committed `SKILL.md` text — none of the three involved driving a real model conversation through a skill's pipeline, so a PASS is discovery/loading/text-identity evidence, not verified end-to-end behavioural evidence that the skill's instructions produce correct model behaviour under that host. Second, Codex advertises a symlinked skill as `<plugin>:<id>`, so these appear as `scratchydisk-skills:<id>`; that is expected, documented under "Skill names under Codex" in [`codex.md`](codex.md), and `spec-to-ship`'s dependency preflight already accepts it. Third, `codex-cli 0.151.0` has no subcommand that expands a skill body — Codex reads `SKILL.md` on demand — so the Codex evidence is discovery plus byte-identical path resolution, not a host-rendered body.

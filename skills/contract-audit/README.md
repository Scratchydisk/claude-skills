...
C1 data-structure fields · C2 vocabularies and casing · C3 cross-module signatures · C4 shared constants and units · C5 ambiguous field meanings · C6 observable acceptance criteria · C7 ownership and invocation · C8 toolchain coherence · C9 example conformance · C10 producer/consumer closure · C11 site completeness

Each is derived from a specific defect that shipped. A FAIL in C1, C2, C3, C7, C9, C10 or C11 blocks implementation — those seven produce silent defects rather than loud ones.

The **silence test** is why those seven block, and the question to ask of any new row: *if the two sides disagreed, would anything fail loudly?* If not — a param silently ignored, a null quietly defaulted, a field `undefined` rather than absent — the contract needs a mechanical conformance check, not a definition and a review.
...
C11 exists because every other row asks whether a thing is *defined*, and none asks *how many of it there are*. A spec saying "the heading" when the codebase has three is wrong about no literal it names — each citation checks out — so C9 passes while the change goes out half-applied, leaving screens that disagree with each other and with the docs. Its inverse is the more expensive half: concluding a site does not exist because the plausibly-named component turned out to only render it. Both are one grep, and neither is reachable by reading.

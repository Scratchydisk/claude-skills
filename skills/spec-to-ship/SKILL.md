...
5. **Plan contract-audit (C9 + C10 + C11).** Invoke `contract-audit` on the plan, scoped to **C9 — example conformance**, **C10 — producer/consumer closure** and **C11 — site completeness**. For C9: every literal in the plan — URL, query string, JSON key, header, env var, CLI flag, identifier in a code snippet or test body — gets diffed against the definition it exercises, including definitions in the spec and in the code the plan targets. For C10: every symbol in a task's `Consumes:` block must appear in some earlier task's `Produces:` block, and every new persisted field the plan reads must have a task that writes it by a named route. That check is a set difference over metadata the plan already carries — do it mechanically, not by reading. For C11, two directions. Outward: every string or symbol the plan says it will change gets grepped, and the number of hits gets compared against the number of files the task lists — a task that edits "the label" in one file while three carry it ships a half-applied rename that no reading catches. **Inward: every enumeration in the SPEC gets diffed against the plan's coverage** — each row of a spec table, each item of a numbered list, each "all N of the following" must map to a task, and you name the ones that do not. Fix every mismatch in the plan, commit, then loop. Scoping to three rows is not a licence to sample: within them every literal gets compared, every consumed symbol gets traced, every changed string gets counted, and every spec enumeration gets diffed.

**Do the inward direction even when `writing-plans` has already claimed spec coverage in its own self-review.** That self-review is the author checking their own work from memory, and its failure mode is skimming: a five-row spec table gets answered with one task name, three rows get implemented, and the two that vanished are wrong nowhere — so every later gate reads clean. Only a set difference finds them, and this is the only stage that performs one.
...
- **Plan contract-audit (C9+C10+C11):** literal mismatches, unproduced symbols, uncounted sites and spec rows with no task — found and fixed, or clean
...
- **Treating the plan audit as being only about what the plan says.** Half of C11 here is about what the plan *fails to say* against the spec that produced it. A plan can be internally flawless and still have dropped a requirement on the way in.

...
- **Entry auto-detection.** Idea only → start at brainstorming. Spec exists → start at the spec contract-audit. Plan exists → start at the plan C9+C10+C11 audit — never at the plan DA-loop directly, which would let an existing plan skip that audit. It states the detected entry point before proceeding.
- **Dependencies loaded by skill ID, never a runtime call.** Each stage loads `brainstorming`, `contract-audit`, `devils-advocate-loop`, `writing-plans`, an implementation skill, or `karpathy-guidelines` natively by ID — see `SKILL.md`'s **Dependencies** section for the per-stage table and provenance. A stage-boundary preflight checks only the dependency the stage about to run actually needs; if that skill is unavailable, the pipeline stops with `The next pipeline stage requires <skill-id>, which is unavailable.` instead of guessing at its behavior.
